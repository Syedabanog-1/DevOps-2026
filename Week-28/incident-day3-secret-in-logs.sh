...
# DevOps 2026 Track
# Scenario: secret-in-logs — diagnose and remediate
# ============================================================
set -euo pipefail
RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; CYAN='\033[0;36m'; BOLD='\033[1m'; RESET='\033[0m'
LAB_DIR="/tmp/devops2026-week28-incident"
RCA_FILE="${LAB_DIR}/RCA-week28-secret-in-logs.md"
mkdir -p "${LAB_DIR}"

print_banner() {
  echo -e "${RED}${BOLD}"
  echo "=========================================================="
  echo "  INCIDENT SIM | Week 28 | secret-in-logs"
  echo "=========================================================="
  echo -e "${RESET}"
}

simulate_incident() {
  echo -e "${RED}[INCIDENT]${RESET} Simulating failure: secret-in-logs"
...
perform_rca() {
  ...
  echo -e "  Conclusion: Root cause identified for scenario: secret-in-logs"
}
...
generate_rca_doc() {
  cat > "${RCA_FILE}" <<RCADOC
# Incident RCA: Pipeline Security and Artifacts
## Week 28 Day 3 | DevOps 2026 Track
...
## Incident Summary
A production failure occurred related to: secret-in-logs.
This is a simulated incident to train engineers to identify and remediate common Pipeline Security and Artifacts failures.

## Root Cause
The failure was triggered by a misconfiguration or a missing dependency in the Pipeline Security and Artifacts layer.
...
main() {
  ...
  apply_fix
  generate_rca_doc
  echo -e "\n${GREEN}${BOLD}  INCIDENT RESOLVED | Week 28 Day 3 — COMPLETE${RESET}"
}
main "$@"
