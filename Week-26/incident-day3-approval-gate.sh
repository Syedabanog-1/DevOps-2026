...
INCIDENT_DIR="/tmp/devops2026-week26-incident"
mkdir -p "${INCIDENT_DIR}"

print_banner() {
  echo -e "${BOLD}${RED}"
  echo "=========================================================="
  echo "  INCIDENT SIMULATION | Week 26 | approval-gate"
  echo "=========================================================="
  echo -e "${RESET}"
}
...
simulate_incident() {
  ...
  echo "FATAL: approval-gate encountered in Azure DevOps Pipelines module." > "${INCIDENT_DIR}/error.log"
...
# Root Cause Analysis (RCA) - Azure DevOps Pipelines

## 📅 Date: $(date)
## 📉 Incident: approval-gate

## 🔍 What happened?
During standard operations of Azure DevOps Pipelines, an unexpected issue 'approval-gate' caused the pipeline to halt.
...
## 🧠 Lesson Learned
Always monitor Azure DevOps Pipelines metrics to proactively catch approval-gate before it affects end users.
EOF
